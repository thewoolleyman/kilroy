...
func (e *Engine) cxdbRunStarted(ctx context.Context, baseSHA string) error {
	...
	if _, err := os.Stat(filepath.Join(e.LogsRoot, "modeldb", "litellm_catalog.json")); err == nil {
		_, _ = e.CXDB.PutArtifactFile(ctx, "", "modeldb/litellm_catalog.json", filepath.Join(e.LogsRoot, "modeldb", "litellm_catalog.json"))
	}
